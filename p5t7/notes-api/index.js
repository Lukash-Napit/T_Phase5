...
// Deliberate 500 simulation (Step 4's requirement) 
v1Router.get('/notes-simulate-error/boom', (req, res, next) => {
  try {
    throw new Error('Simulated unexpected server error');
  } catch (err) {
    next(err); // deliberately NOT a NotFoundError/ValidationError -- no statusCode
  }
});
...
app.use((err, req, res, next) => {
  ...
  res.status(statusCode).json({
    error: {
      ...
      code: statusCode === 500 ? 'InternalError' : err.name || 'Error',
...
// Test-only helper: resets in-memory state between test cases.
app.resetForTests = () => {
  notes = [];
  nextId = 1;
};
