# Notes API — A5: REST API Design

Full-featured Notes API: pagination, filtering, boundary validation, and centralized error handling, all verified with an automated test suite.
...
npm test
```
Runs 9 automated tests covering pagination boundaries, filtering+pagination interaction, and error cases. Should complete cleanly with `9 pass, 0 fail`.

## Pagination

`GET /v1/notes?page=1&limit=10` — both optional, defaulting to `page=1, limit=10`. Response:
```json
{
  "data": [...],
...
```
- A page beyond available data returns `200` with an empty `data` array — not an error.
- Invalid values (`page=-1`, `limit=abc`) are rejected with `400` and a specific field-level message — never silently defaulted or crashed on.

## Filtering (2 distinct types, combinable with pagination)

- **Boolean flag**: `GET /v1/notes?completed=true`
- **Text search**: `GET /v1/notes?search=milk` (case-insensitive substring match on note text)
- Both can combine with pagination: `GET /v1/notes?completed=true&page=2&limit=5`

**Critical detail, verified by test**: `total`/`totalPages` are always calculated from the **filtered** result set, not the full unfiltered dataset — e.g. with 15 notes total and only 5 completed, `?completed=true` reports `total: 5`, never `total: 15`.

## Postman collection

See `notes-api.postman_collection.json` — covers every endpoint with realistic example data (not placeholders), including a documented `400` validation error response and a documented `400` invalid-pagination response, so the error envelope shape itself is part of the documentation.
